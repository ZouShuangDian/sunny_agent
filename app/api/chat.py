...
async def _build_mode_intent(
    ...
    # 设置 ModeContext ContextVar（从 ModeConfig 透传 allowed_tools + override_system_prompt）
...
async def chat(
    ...
    if langfuse:
        ...
        # start_as_current_span 设置 OTEL 上下文，LiteLLM 的 langfuse_otel 回调
        # 会自动将 LLM 调用挂为子 span，usage 聚合到同一 trace
        _langfuse_cm = langfuse.start_as_current_span(
            name="chat_request",
            input={"message": body.message},
            metadata={"source": "api", "model": settings.LLM_DEFAULT_MODEL},
        )
        langfuse_trace = _langfuse_cm.__enter__()
        langfuse.update_current_trace(
            user_id=user.usernumb,
            session_id=session_id,
            input={"message": body.message},
        )
        langfuse_trace_var.set(langfuse_trace)

    # propagate_attributes 确保子 span 继承 user_id/session_id（Langfuse v3 OTEL 传播）
    with (
        _propagate_attributes(
            user_id=str(user.usernumb),
            session_id=session_id,
        ...
        try:
            ...
            if langfuse_trace:
                langfuse_trace.update(status_message=str(exc), level="ERROR")
            raise
        finally:
            if langfuse_trace:
                langfuse_trace.update_trace(output={"reply": reply_text} if 'reply_text' in dir() else {})
                if _langfuse_cm:
                    _langfuse_cm.__exit__(None, None, None)
                langfuse.flush()
            langfuse_trace_var.set(None)
    ...
    async def event_generator():
        ...
        if langfuse:
            ...
            _langfuse_cm = langfuse.start_as_current_span(
                name="chat_request",
                input={"message": body.message},
                metadata={"source": "api_stream", "model": settings.LLM_DEFAULT_MODEL},
            )
            langfuse_trace = _langfuse_cm.__enter__()
            langfuse.update_current_trace(
                user_id=user.usernumb,
        ...
        try:
            ...
            if langfuse_trace and not body.session_id:
                langfuse_trace.update_trace(session_id=session_id)
            ...
            if langfuse_trace:
                langfuse_trace.update(status_message=str(e), level="ERROR")
            ...
            if langfuse_trace:
                langfuse_trace.update_trace(
                    output={"reply": "".join(reply_chunks)} if 'reply_chunks' in dir() and reply_chunks else {}
                )
                if _langfuse_cm:
                    _langfuse_cm.__exit__(None, None, None)
                langfuse.flush()
